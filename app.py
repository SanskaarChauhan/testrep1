...
st.set_page_config(page_title="SecureShield WAF", layout="wide")
...
for key, default in [
    ("pre_ip",      "192.168.1.1"),
    ("pre_url",     "/login"),
    ("pre_body",    "admin' OR 1=1 --"),
    ("run_preset",  False),
]:
    if key not in st.session_state:
        st.session_state[key] = default

st.markdown("## SecureShield WAF — Model Comparison Dashboard")
st.markdown(
    "Hybrid model (Burp Suite + Metasploit) always runs on the left. "
    "Select any individual model from the dropdown to compare on the right."
)
st.divider()

with st.expander("Request Input + Model Selection", expanded=True):
    col_inp, col_sel = st.columns([3, 1])

    with col_inp:
        ip   = st.text_input("Source IP Address",       value=st.session_state["pre_ip"])
        url  = st.text_input("Request URL",              value=st.session_state["pre_url"])
        body = st.text_area("Request Body / Payload",    value=st.session_state["pre_body"], height=120)
    ...
    with col_sel:
        ...
        for label, (pip, purl, pbody) in PRESETS.items():
            if st.button(label, key=f"preset_{label}", use_container_width=True):
                st.session_state["pre_ip"]     = pip
                st.session_state["pre_url"]    = purl
                st.session_state["pre_body"]   = pbody
                st.session_state["run_preset"] = True
                st.rerun()

scan_clicked = st.button("Scan and Compare", type="primary", use_container_width=True)
run_scan = scan_clicked or st.session_state["run_preset"]
...
if "hybrid_result" in st.session_state:
    ...
    s1.metric("Hybrid verdict",     "BLOCKED" if hr["blocked"] else "ALLOWED")
    s2.metric(f"{smn} verdict",     "BLOCKED" if sr["blocked"] else "ALLOWED")
    s3.metric("Hybrid confidence",  f"{hr['confidence']:.1f}%")
    s4.metric(f"{smn} confidence",  f"{sr['confidence']:.1f}%")

    st.divider()

    left, gap, right = st.columns([10, 1, 10])
    ...
    with right:
        ...
        mc = model_colors.get(smn, "#e2e8f0")
    ...
    for metric_name, (hval, sval, maxval) in metrics.items():
        if metric_name == "Scan time (ms)":
            hbar = round((1 - hval / maxval) * 100) if maxval else 100
            sbar = round((1 - sval / maxval) * 100) if maxval else 100
        else:
            hbar = round(hval / maxval * 100) if maxval else 0
            sbar = round(sval / maxval * 100) if maxval else 0
        ...
        <div class="comp-bar-wrap">
            ...
            <div class="comp-bar-row">
                ...
                <div class="comp-bar-track"><div class="comp-bar-fill" style="width:{hbar}%;background:#00d4ff"></div></div>
                <div class="comp-bar-val"  style="color:#00d4ff">{round(hval, 2)}</div>
            </div>
            <div class="comp-bar-row">
                <div class="comp-bar-name" style="color:{mc}">{smn}</div>
                <div class="comp-bar-track"><div class="comp-bar-fill" style="width:{sbar}%;background:{mc}"></div></div>
    ...
    for col, title, winner, value in [
        (w1, "HIGHEST CONFIDENCE", conf_winner,   f"{max(hr['confidence'],   sr['confidence']):.1f}%"),
        (w2, "MOST THREATS FOUND", threat_winner, f"{max(hr['threat_count'], sr['threat_count'])} threats"),
        (w3, "FASTEST SCAN",       speed_winner,  f"{min(hr['scan_time_ms'], sr['scan_time_ms']):.2f} ms"),
    ]:
        color = "#00d4ff" if winner == "Hybrid" else mc
        col.markdown(f"""
        <div style='text-align:center;padding:14px;background:#0d1117;border-radius:8px;border:1px solid rgba(255,255,255,0.07)'>
            ...
            <div style='font-size:18px;font-weight:700;color:{color}'>{winner}</div>
            <div style='font-size:12px;color:#64748b'>{value}</div>
        </div>""", unsafe_allow_html=True)

with st.sidebar:
    st.markdown("### WAF Status")
    ...
    st.divider()
    if st.button("Clear blocked IPs", use_container_width=True):
